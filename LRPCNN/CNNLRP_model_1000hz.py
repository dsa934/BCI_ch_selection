...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        # after conv1 -> shape = [10, 20, 154, 22]
        x = self.relu(self.max_pool1(self.conv1(x)))
        #print("X",x.shape)

        # after conv2 -> shape = [10, 30, 123, 10]
...
def val(args, model, device, val_loader):
    ...
    #val_loss = torch.tensor(val_loss).to(torch.long)
    #val_correct = torch.tensor(val_correct)
