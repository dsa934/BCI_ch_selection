...
from tool_utils import pprint, Flatten
...
class RelevancePropagator:
    ...
    def linear_inverse(self, m, relevance_in):
        ...
        if self.method == "b-rule":
            ...
            input_relevance[:, :2*out_c] *= (1+self.beta)/norm[:, :out_c].repeat(1, 2).double()
            input_relevance[:, 2*out_c:] *= -self.beta/norm[:, out_c:].repeat(1, 2).double()
            inv_w = w.t()
    ...
    def conv_nd_inverse(self, m, relevance_in):
        ...
        if self.method == "b-rule":
            with torch.no_grad():
                ...
                input_relevance[:, :2*out_c] *= (1+self.beta)/norm[:, :out_c].repeat(1, 2, *spatial_dims).double()
                input_relevance[:, 2*out_c:] *= -self.beta/norm[:, out_c:].repeat(1, 2, *spatial_dims).double()
                ...
                relevance_out = conv_nd(relevance_out, weight=sum_weights.double(), bias=None)
